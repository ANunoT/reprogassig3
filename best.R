...
# exist, and if the requested outcome was invalid. Check to ensure that two character state code id correct. Load data into data frame.

OutcomeData <- read.csv("C:/Users/Nuno/Documents/R-Programming/rprog_data_ProgAssignment3-data/outcome-of-care-measures.csv")

# lets get a list of unique state, then check if the state argument exists. The argument is forced to upper case, ensuring no issues with capitalisation.
uniqueState <- unique(OutcomeData[,7])

        if (!toupper(stateCode) %in% uniqueState) stop(badState)


## At this stage all ready pro processing. 

return("ready for precessing")
